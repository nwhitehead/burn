...
//            println!("z_t = {:?}", update_values.clone());
            ...
            // n(ew)g(ate) tensor
            let biased_ng_input_sum = self.gate_product(&input_t, &hidden_t, Some(&reset_values), &self.new_gate);
...
//                println!("gate_product hidden_product = {:?}  @  {:?}", hidden.clone(), gate.hidden_transform.weight.val().clone());
            ...
            (Some(input_bias), None, None) => input_product + input_bias.unsqueeze() + hidden_product,
            (None, Some(hidden_bias), Some(r)) => input_product + r.clone().mul(hidden_product + hidden_bias.unsqueeze()),
            (None, Some(hidden_bias), None) => input_product + hidden_product + hidden_bias.unsqueeze(),
